...
| **스킬** | 매뉴얼 | 작업 절차서 — 여러 단계 자동화 (Part 6에서 본격 제작) |

오늘은 그중 **두 개**를 만듭니다. 커맨드 하나 + 서브에이전트 하나. 매뉴얼(스킬)은 Part 6에서 본격 만들어요.
...
먼저 강의 워크스페이스에 이미 있는 `/stuck` 커맨드를 봅니다. 안티그래비티 에디터에서 `~/fastcampus-cc/.claude/commands/stuck.md`를 열어보세요. 안 내용이 이렇게 생겼어요.
...
파일이 만들어졌으면 진짜로 동작하는지 확인. **여기서 핵심은 직접 호출이 아니라 자연어 자동 위임이에요.** `/practice-coach`라고 안 부르고, 그냥 자연어로 부탁해봅니다.
